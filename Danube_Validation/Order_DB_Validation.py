...
from source_and_destination_table import orderDataBaseValidations
import os
import psycopg2
...
def validate_tables(config1, config2):
    ...
    try:
        ...
        for validation in orderDataBaseValidations:
...
if __name__ == "__main__":
    V1_config = {
        'dbname': os.getenv('dbname5'),
        'user': os.getenv('user5'),
        'password': os.getenv('password5'),
        'host': os.getenv('host5'),
        'port': os.getenv('port5')  # Fixed the key for port1
    }
    ...
    validation_results.to_csv('D:/DB_Results/order_db.csv', index=False)
