...
from source_and_destination_table import productDataBaseValidations
import os
import psycopg2
...
def validate_tables(config1, config2):
    ...
    try:
        ...
        for validation in productDataBaseValidations:
...
if __name__ == "__main__":
    V1_config = {
        'dbname': os.getenv('dbname3'),
        'user': os.getenv('user3'),
        'password': os.getenv('password3'),
        'host': os.getenv('host3'),
        'port': os.getenv('port3')  # Fixed the key for port1
    }
    ...
    validation_results.to_csv('D:/DB_Results/product_db.csv', index=False)
